...
const AddTask = ({handleNewTaskAdded}) => {
  ...
  const addTask = async () => {
    if(newTaskTitle.trim()) {
      ...
      toast.error("Bạn cần nhập nhiệm vụ");
    }
  };
  ...
  return (
    <Card className="p-6 border-0 bg-gradient-card shadow-custom-lg">
      <div className='flex flex-col gap-3 sm:flex-row'>
        ...
        />
        <Button variant="gradient" size="xl" className="px-6" onClick={addTask} disabled={!newTaskTitle.trim()}><Plus className='size-5'/>Thêm </Button>
      </div>
    </Card>
  )
}
